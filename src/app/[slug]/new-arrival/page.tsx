...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${name}'s New Arrivals`,
    description: `Just-added products from ${name} on ${brand.name}.`,
    alternates: { canonical: `${siteUrl}/${slug}/new-arrival` },
    openGraph: {
      title: `${name}'s New Arrivals`,
      description: `Just-added products from ${name} on ${brand.name}.`,
      url: `${siteUrl}/${slug}/new-arrival`,
      siteName: brand.name,
    },
  };
}

export default async function NewArrivalSlugPage({ params }: Props) {
  const { slug } = await params;
  const data = await loadAskBootstrap(slug);
  if (data.accountType !== "brand") redirect(`/${slug}/ask`);
  return <AskPage {...data} initialView="newarrival" />;
}
